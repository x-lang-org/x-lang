...
Given an array of **distinct** integers `candidates` and a target integer `target`, return <em>a list of all **unique combinations** of </em>`candidates`<em> where the chosen numbers sum to </em>`target`<em>.</em> You may return the combinations in **any order**.
