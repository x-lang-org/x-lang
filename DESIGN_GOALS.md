...
## 12. 精简标准库：语言特性优先，平台寄生其余

X 语言采用**精简标准库**设计：只提供语言核心特性必须的类型，其他功能由目标平台的标准库提供。

### 标准库内容（语言特性必须）

- **Option&lt;T&gt;**：替代 null，表示可能存在或不存在的值
- **Result&lt;T, E&gt;**：替代异常，表示可能成功或失败的操作
- **Prelude**：`print`、`println`、`panic`、`assert`、`todo` 等基础函数

这些类型和函数是语言特性的核心部分（如 `?` 运算符、模式匹配），必须由标准库提供。
...
| 集合 | `std.ArrayList` | Array/Map/Set | `java.util.*` | `System.Collections.*` |

**平台特定导入**：如果 X 代码导入特定平台的库（如 `import java.util.ArrayList`），则该代码只能编译运行于对应平台（JVM）。

### 设计准则

1. **语言特性优先**：只包含语言特性必须的类型（Option、Result）
2. **不重复造轮子**：目标平台已有成熟的标准库，无需封装
3. **零开销互操作**：直接调用平台 API，无封装层开销
...
| 11 | C FFI        | 与 C 零开销互操作 |
| 12 | 精简标准库   | 语言特性必须的 Option/Result + 平台寄生其余 |
| 13 | 多后端       | Zig / LLVM / JS / JVM / .NET，一次编写多处运行 |
| 14 | 无异常       | `Option` + `Result` + `?`，错误即值 |
