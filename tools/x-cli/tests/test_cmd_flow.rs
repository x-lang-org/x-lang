...
fn documented_integration_category_form_runs_requested_category_only() {
    ...
    assert!(
        stdout.contains("5 passed; 0 failed; 0 skipped; 5 total"),
        "stdout was: {stdout}"
    );
}
...
fn non_integration_extra_positional_is_rejected() {
    ...
    assert!(
        stderr.contains("额外的测试分类 `extra` 仅支持与 `x test integration <category>` 一起使用"),
        "stderr was: {stderr}"
    );
}
...
fn stdlib_integration_suite_reports_stdin_io_case() {
    ...
    assert!(
        stdout.contains("io_unicode_test ... ok"),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("io_whitespace_test ... ok"),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("6 passed; 0 failed; 0 skipped; 6 total"),
        "stdout was: {stdout}"
    );
}
