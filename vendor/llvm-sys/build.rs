...
/// On Windows, when llvm-config is not present, use LLVM_SYS_*_PREFIX include/lib
/// directly (e.g. official LLVM installer does not ship llvm-config.exe).
/// Returns true if configuration was done and build should continue without
/// setting LLVM_SYS_NOT_FOUND.
fn use_windows_prefix_fallback() -> bool {
    if !target_os_is("windows") || !target_env_is("msvc") {
        return false;
    }
    let prefix = match env::var_os(&*ENV_LLVM_PREFIX) {
    ...
    if !include_dir.is_dir() || !lib_dir.is_dir() {
        println!(
            "cargo:warning=LLVM prefix {:?} missing include/ or lib/; skipping prefix fallback",
            prefix
        );
        return false;
    }
    // Windows installer often ships only a subset of headers (e.g. lto.h, Remarks.h) and
    // omits llvm-c/Target.h etc. Require the header we need for the wrapper.
    if !include_dir.join("llvm-c").join("Target.h").exists() {
        println!(
            "cargo:warning=LLVM prefix {:?} has no include/llvm-c/Target.h (Windows installer often omits dev headers). Install full LLVM SDK or use WSL. Skipping prefix fallback.",
            prefix
        );
        return false;
    }

    // Avoid inheriting CFLAGS that might contain Unix-style flags (e.g. -W4) which cl.exe rejects.
    std::env::remove_var("CFLAGS");

    // Build the extra wrapper functions (same as llvm-config path).
    if !cfg!(feature = "disable-alltargets-init") {
        cc::Build::new()
            .warnings(false) // cc passes -W4 to MSVC but cl.exe expects /W4; avoid that
    ...
    if cfg!(feature = "no-llvm-linking") {
        println!("cargo:config_path={}", prefix.display());
        return true;
    }
    ...
    // Discover all .lib files in prefix/lib (Windows MSVC import libs for DLLs).
    let libs: Vec<String> = match std::fs::read_dir(&lib_dir) {
        Ok(rd) => rd
            .filter_map(|e| e.ok())
            .filter(|e| e.path().extension().map_or(false, |ext| ext == "lib"))
            .filter_map(|e| e.path().file_stem().and_then(|s| s.to_owned().into_string().ok()))
            .collect(),
        Err(e) => {
            println!("cargo:warning=reading lib dir {}: {}", lib_dir.display(), e);
            Vec::new()
        }
    };
    for name in &libs {
    ...
    // System libraries commonly needed by LLVM on Windows MSVC.
    for name in ["psapi", "shell32", "ole32"] {
        println!("cargo:rustc-link-lib=dylib={}", name);
    }

    if env::var_os(&*ENV_USE_DEBUG_MSVCRT).is_some() {
        println!("cargo:rustc-link-lib=msvcrtd");
    }

    if env::var_os(&*ENV_FORCE_FFI).is_some() {
...
fn main() {
    ...
    let llvm_config_path = match locate_llvm_config() {
        None => {
            if use_windows_prefix_fallback() {
                return;
            }
