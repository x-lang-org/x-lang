...
| 逻辑运算 | and, or, not (或 &&, \|\|, !) | ✅ |
| 位运算 | &, \|, ^, ~ | ✅ |
| 字符串插值 | "Hello, $name!" | ⚠️ 未实现 | 示例未使用，可选功能 |
| 管道运算符 | \|> | ✅ |
...
1. **缺少的关键字** (~10个): `mutable`, `constant`, `each`, `then`, `break`, `continue`, `record`, `constructor`, `concurrently`, `perform`, `operation`

2. **类型名称**: 编译器内部使用大写，用户代码可使用小写（有映射）

3. **宏语法**: 使用 `println(...)` 而非 `println!(...)`

4. **字符串插值**: 尚未实现 `${expr}` 语法

5. **for 循环**: 使用 `for x in` 而非 `for each x in`
...
### 高优先级
1. **添加缺失的关键字**: `mutable`, `constant`, `each`, `then`, `break`, `continue`, `record`, `constructor`, `concurrently`, `perform`, `operation`
2. **字符串插值**: 实现 `${expr}` 语法

### 中优先级
3. 循环语法支持 `for each x in`
4. 宏调用语法统一 (`println!` vs `println`)
5. 添加 `then` 关键字支持 `if then else` 语法
