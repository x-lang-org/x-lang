...
X 是一门**现代的、以通用用途为目标的编程语言**。它目前已经展现出从底层到上层多个方向的设计意图，但不同子系统与后端的成熟度仍不均衡；现阶段更适合作为一个快速演进中的语言实现来理解。它的设计融合了多种编程范式的优点，旨在提供一种既安全又高效的开发体验。
...
X 不是领域特定语言（DSL），而是以通用语言为目标，面向从底层系统到上层应用的广泛场景；不过不同能力与后端的成熟度仍在持续演进中。当前更适合作为一个正在快速走向生产价值的实现来理解，而不是一个所有方向都已完成验证的成品。适用方向包括：
...
9. **持续完善的工具链目标**：`x` CLI 以 Cargo 级体验为目标，当前已具备本地开发闭环并持续补齐生态能力
